...
execute as @a[scores={freezeTimer=1200..}] at @s positioned ~ ~-1 ~ run tp @e[type=allay, tag=ghost, sort=nearest, limit=1, distance=..1] ~ ~2 ~ ~ ~
execute as @a[scores={freezeTimer=1200..}] run tag @e[type=skeleton, tag=highlighter, sort=nearest, limit=1] add locked
